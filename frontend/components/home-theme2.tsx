...
export interface MarketStore {
  ...
  productCount: number
  theme?: string
  /** Posicionamiento del superadmin: >0 = comercio destacado (1 = primero). 0/undefined = normal. */
  marketplaceOrder?: number | null
...
function StoreCard({
  ...
  const isFeatured = !!(store.marketplaceOrder && store.marketplaceOrder > 0)
  return (
    <button
    ...
    >
      ...
      {isFeatured && (
        <span className={`absolute ${hasServices ? 'top-11' : 'top-2.5'} left-2.5 z-30 flex items-center gap-1 text-[9px] sm:text-[10px] font-extrabold px-2 py-1 rounded-full backdrop-blur-sm bg-amber-400/90 text-black border border-amber-300 shadow-sm uppercase tracking-wide`}>
          ★ Destacado
        </span>
      )}
...
export function MarketplaceHomeGovCo({
  ...
  const visibleStores = useMemo(() =>
    stores
      ...
      .sort((a, b) => {
        // 1) Posicionamiento del superadmin: comercios destacados (marketplaceOrder > 0) primero,
        //    en el orden asignado (1, 2, 3…). El resto (0/undefined) queda después.
        const ao = a.marketplaceOrder && a.marketplaceOrder > 0 ? a.marketplaceOrder : Infinity
        const bo = b.marketplaceOrder && b.marketplaceOrder > 0 ? b.marketplaceOrder : Infinity
        if (ao !== bo) return ao - bo
        // 2) Entre iguales, los comercios con productos van antes que los vacíos.
