...
Migración 0034: `share_links` (code único, type product/store/collection, config JSON, title, clicks). Backend superadmin CRUD + público `GET /storefront/share/:code` (resuelve + suma clic, inactivo→404). Producto y tienda reusan deep-links existentes (`?product=`, `/t/<slug>`); lo nuevo es la colección filtrada por rubro+comercios. E2E 14/14, tsc back 6 (0 nuevos). **Fase 2 ✅** ruta `app/l/[code]` (redirige product/store/collection) + `?collection=` filtra `stores` a los de la colección (rubro/comercios) + banner de título. E2E 7/7, tsc front 8 (0 nuevos). **Fase 3 ✅** generador panel superadmin (tab "Links": crear los 3 tipos con rubro/comercios, QR descargable con qrcode.react, copiar URL, clics, activar/borrar; slug añadido a tenants). tsc back 6 / front 8 (0 nuevos). **🎉 ROADMAP LINKS COMPLETO (3/3). Pendiente commit + redeploy.**
