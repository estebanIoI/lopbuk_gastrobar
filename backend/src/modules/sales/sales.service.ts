  ...
  async create(tenantId: string, data: CreateSaleData): Promise<Sale> {
    ...
    try {
      ...
      if (data.customerPhone) {
        ...
        // Fidelización: acreditar puntos por la venta presencial (best-effort, ya commiteada).
        // Server-side → funciona para cualquier rol de cajero (no depende de permisos del POS)
        // y no duplica con el frontend. earnPoints respeta loyalty_config.enabled.
        import('../loyalty/loyalty.routes').then(({ earnPoints }) =>
          earnPoints(tenantId, data.customerPhone as string, data.customerName || undefined, total, saleId)
        ).catch(() => {});
      }
